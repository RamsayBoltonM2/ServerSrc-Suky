				...
				if (m_pointsInstant.bItemGrid[bCell] == (UINT)iExceptionCell)
				{
					...
					do
					{
						...
						if (m_pointsInstant.bItemGrid[p])
							if (m_pointsInstant.bItemGrid[p] != (UINT)iExceptionCell)
								return false;
					}
					while (++j < bSize);
					...
					case USE_POTION_NODELAY:
						{
							...
							if (used)
							{
								...
								LastPotionNoDelayUse = thecore_pulse();
							}
						}
						break;
					}
...
bool CHARACTER::DropItem(TItemPos Cell, short bCount)
{
	...
	if (bCount == 0 || (unsigned short) bCount > item->GetCount())
	...
	if ((unsigned short) bCount == item->GetCount())
	{
		item->RemoveFromCharacter();
		pkItemToDrop = item;
	}
...
bool CHARACTER::MoveItem(TItemPos Cell, TItemPos DestCell, short count)
{
	...
	else
	{
		...
		if (count == 0 || (unsigned short) count >= item->GetCount() || !item->IsStackable() || IS_SET(item->GetAntiFlag(), ITEM_ANTIFLAG_STACK))
		{
		...
		else if ((unsigned short) count < item->GetCount())
		{
				...
				if (item->IsStackable() && !IS_SET(item->GetAntiFlag(), ITEM_ANTIFLAG_STACK))
				{
					...
					for (int i = 0; i < INVENTORY_MAX_NUM; ++i)
					{
						...
						if (item2->GetVnum() == item->GetVnum())
						{
							...
							BYTE bCount2 = MIN(ITEM_MAX_COUNT - item2->GetCount(), bCount);
							bCount -= bCount2;
				...
				if (item->IsStackable() && !IS_SET(item->GetAntiFlag(), ITEM_ANTIFLAG_STACK))
				{
					...
					for (int i = 0; i < INVENTORY_MAX_NUM; ++i)
					{
						...
						if (item2->GetVnum() == item->GetVnum())
						{
							...
							BYTE bCount2 = MIN(250 - item2->GetCount(), bCount);
							bCount -= bCount2;
							...
							if (bCount == 0)
							{
								...
								ChatPacket(CHAT_TYPE_INFO, LC_TEXT("아이템 전�?: %s 님에게 %s"), owner->GetName(), item2->GetName());
								M2_DESTROY_ITEM(item);
								if (item2->GetType() == ITEM_QUEST)
...
bool CHARACTER::EquipItem(LPITEM item, int iCandidateCell)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Horse> Dont change armor on horse."));
		return false;
	}
