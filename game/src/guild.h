...
typedef struct SGuildMember
{
	...
	BYTE job;
	int level;
	long long offer_exp; // 공헌한 경험치
	BYTE _dummy;

	std::string name;

	SGuildMember(LPCHARACTER ch, BYTE grade, long long offer_exp);
	SGuildMember(DWORD pid, BYTE grade, BYTE is_general, BYTE job, int level, long long offer_exp, char* name);
...
class CGuild
{
	public:
		...
		bool		OfferExp(LPCHARACTER ch, long long amount);
		...
		void		GuildPointChange( BYTE type, long long amount, bool save = false );
