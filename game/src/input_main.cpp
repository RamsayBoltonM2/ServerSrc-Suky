...
int CheckItem(std::string itemChat,LPCHARACTER ch)
{
	...
	for(int i = 0; i < results2.size(); i++)
	{
		...
		if (results2[i].find("item") != std::string::npos)
		{
			...
			//sys_log(0, "INFO SIZE: %d, RESULT_SIZE: %d", item_info.size(), results2.size());
			
			if (item_info.size() < 4 + 2 || results2.size() < 4)
				return 4;
...
int CInputMain::Whisper(LPCHARACTER ch, const char * data, size_t uiBytes)
{
	...
	if (ch->GetPMCounter() > 5 && ch->GetLastPMPulse() > thecore_pulse())
	{
	...
	else
	{
		...
		else
		{
			...
			if(err==4){
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Occurred during the parsing of information about the subject, the message has not been sent!"));
				return iExtraLen;
			}
			if(err==2){
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("There parsing the message, the message has not been sent!"));
				return iExtraLen;
			}
		...
		case CHAT_TYPE_TEAM:
			{
				...
				for (iter=gm_list.begin(); iter!=gm_list.end();++iter)
				{
					...
					if (receiver != NULL)
						receiver->ChatPacket(CHAT_TYPE_TEAM, chatbuf);
				}
...
int CInputMain::Shop(LPCHARACTER ch, const char * data, size_t uiBytes)
{
	...
	switch (p->subheader)
	{
		...
		case SHOP_SUBHEADER_CG_SELL2:
			{
				if (uiBytes < sizeof(UINT) + sizeof(short))
					return -1;
		...
		case HEADER_CG_ATTACK:
			{
				...
				if (victim->IsPC() && victim->GetMyShop())
					return;
				
				if (victim->IsPC() && victim->GetMining())
					return;
				
				if (victim->IsPC() && victim->GetFishing())
					return;
...
int CInputMain::MyShop(LPCHARACTER ch, const char * c_pData, size_t uiBytes)
{
	...
	int iExtraLen = p->bCount * sizeof(TPrivateShopItemTable);
	...
	ch->OpenMyShop(p->szSign, (TPrivateShopItemTable *) (c_pData + sizeof(TPacketCGMyShop)), p->bCount);
	return (iExtraLen);
}
