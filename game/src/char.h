...
//		bool			IsPet() const		{ return m_bCharType == CHAR_TYPE_PET; }
		...
		void			SetHP(long long hp)		{ m_points.hp = hp; }
		long long		GetHP() const		{ return m_points.hp; }

		void			SetSP(long long sp)		{ m_points.sp = sp; }
		long long		GetSP() const		{ return m_points.sp; }

		void			SetStamina(int stamina)	{ m_points.stamina = stamina; }
		int				GetStamina() const		{ return m_points.stamina; }

		void			SetMaxHP(long long iVal)	{ m_pointsInstant.iMaxHP = iVal; }
		long long		GetMaxHP() const	{ return m_pointsInstant.iMaxHP; }

		void			SetMaxSP(long long iVal)	{ m_pointsInstant.iMaxSP = iVal; }
		long long		GetMaxSP() const	{ return m_pointsInstant.iMaxSP; }

		void			SetMaxStamina(int iVal)	{ m_pointsInstant.iMaxStamina = iVal; }
		int				GetMaxStamina() const	{ return m_pointsInstant.iMaxStamina; }

		void			SetRandomHP(long long v)	{ m_points.iRandomHP = v; }
		void			SetRandomSP(long long v)	{ m_points.iRandomSP = v; }

		long long		GetRandomHP() const	{ return m_points.iRandomHP; }
		long long		GetRandomSP() const	{ return m_points.iRandomSP; }

		long long		GetHPPct() const;
	...
	public:
		...
		void			OpenMyShop(const char * c_pszSign, TPrivateShopItemTable * pTable, short bItemCount);
	...
	public:
		void			SetMining(LPCHARACTER ch) { m_pkMining = ch; }
		LPCHARACTER		GetMining() const { return m_pkMining;}
		void			SetFishing(LPCHARACTER ch) { m_pkFishing = ch; }
		LPCHARACTER		GetFishing() const { return m_pkFishing; }
		
	protected:
		LPCHARACTER		m_pkMining;
		LPCHARACTER		m_pkFishing;
