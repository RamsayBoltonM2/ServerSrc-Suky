...
#include "questmanager.h"
	...
	SGuildMember::SGuildMember(LPCHARACTER ch, BYTE grade, long long offer_exp)
: pid(ch->GetPlayerID()), grade(grade), is_general(0), job(ch->GetJob()), level(ch->GetLevel()), offer_exp(offer_exp), name(ch->GetName())
{}
	SGuildMember::SGuildMember(DWORD pid, BYTE grade, BYTE is_general, BYTE job, int level, long long offer_exp, char* name)
...
void CGuild::AddMember(TPacketDGGuildMember * p)
{
	...
	sys_log(0, "GUILD: AddMember PID %u, grade %u, job %u, level %d, offer %lld, name %s ptr %p",
...
void CGuild::LoadGuildMemberData(SQLMsg* pmsg)
{
	...
	for (uint i = 0; i < pmsg->Get()->uiNumRows; ++i)
	{
		...
		BYTE level = (int)strtoul(row[4], (char**) NULL, 10);
...
{
	DBManager::instance().Query("UPDATE guild%s SET level=%d, exp=%lld, skill_point=%d WHERE id = %u", get_table_postfix(), m_data.level,m_data.exp, m_data.skill_point,m_data.guild_id);
}
...
bool CGuild::OfferExp(LPCHARACTER ch, long long amount)
{
	...
	if (ch->GetExp() < (long long) amount)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<길드> 제공하고자 하는 경험치가 남은 경험치보다 많습니다."));
		return false;
	}

	if (ch->GetExp() - (long long) amount > ch->GetExp())
	{
		sys_err("Wrong guild offer amount %lld by %s[%u]", amount, ch->GetName(), ch->GetPlayerID());
		return false;
	}
...
static DWORD __guild_levelup_exp(long long level)
{
...
void CGuild::SaveMember(DWORD pid)
{
	...
	DBManager::instance().Query(
			"UPDATE guild_member%s SET grade = %d, offer = %lld, is_general = %d WHERE pid = %u and guild_id = %u",
...
int CGuild::GetMaxMemberCount()
{
	...
	return 40 + 3 * (m_data.level-1) + m_iMemberCountBonus;
}
