...
namespace quest
{
	...
	int pet_add_affect(lua_State* L)
	{
		...
		if( NULL != ch )
		{
			ch->AddAffect(AFFECT_PET, aApplyInfo[applyOn].bPointType, value, AFF_NONE, duration, 0, false);
		}

		return 0;
	}
	
	int pet_remove_all_affect( lua_State* L )
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();

		if ( ch != NULL )
		{
			ch->RemoveAffect(AFFECT_PET);
		}

		return 0;
	}
	
	int pet_get_effect(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		CPetSystem* petSystem = ch->GetPetSystem();
	...
	void RegisterPetFunctionTable()
	{
		luaL_reg pet_functions[] =
		{
			...
			{ "is_mine",		pet_is_mine		},
			{ "get_effect",	pet_get_effect					},
			{ "affect_add",	pet_add_affect					},
			{ "affect_remove_all",	pet_remove_all_affect	},
			{ NULL,				NULL				}
		};
