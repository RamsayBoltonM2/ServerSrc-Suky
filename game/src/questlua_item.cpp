...
namespace quest
{
	...
	int item_create(lua_State* L)
	{
		...
		BYTE count = lua_tonumber(L, 2);
		
		CItem * item = ITEM_MANAGER::instance().CreateItem(vnum, count, 0, false, 0, false);
	...
	int item_to_character(lua_State* L)
	{
		...
		if (!bCell)
			...
			cell = (BYTE) lua_tonumber(L, 1);
			
			if (pc->GetItem(TItemPos(cell, INVENTORY)) || !pc->IsEmptyItemGrid(TItemPos(cell, INVENTORY), item->GetSize(), (int) cell)) {
				lua_pushboolean(L, false);
				return 1;
			}
		}
	...
	int item_to_equipment(lua_State* L)
	{
		CHARACTER * pc = quest::CQuestManager::instance().GetCurrentCharacterPtr();
		CItem * item = quest::CQuestManager::instance().GetCurrentItem();
	...
	int item_to_ground(lua_State* L)
	{
		CHARACTER * pc = quest::CQuestManager::instance().GetCurrentCharacterPtr();
		CItem * item = quest::CQuestManager::instance().GetCurrentItem();
		
		if (!lua_tonumber(L, 1) || !lua_tonumber(L, 2) || !lua_tonumber(L, 3) || !lua_tonumber(L, 4)|| !lua_tonumber(L, 5) || !item || !pc) {
			lua_pushboolean(L, false);
			return 1;
		}
		...
		int ownershipTime = lua_tonumber(L, 4);
		int destroyTime = lua_tonumber(L, 5);
	...
	int item_get_attribute_type(lua_State* L)
	{
		...
		if (index < 0 || index >= 7)
		{
			sys_err("index(%d) is out of range (0..%d)", index, 7);
			lua_pushnumber(L, 0);
		}
		else
			lua_pushnumber(L, item->GetAttributeType(index));
	...
	int item_get_attribute_value(lua_State* L)
	{
		...
		if (index < 0 || index >= 7)
		{
			...
			lua_pushnumber(L, item->GetAttributeValue(index));
