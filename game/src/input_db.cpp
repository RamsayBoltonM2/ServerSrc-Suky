...
void CInputDB::PlayerCreateSuccess(LPDESC d, const char * data)
{
	...
	// const int MAX_INITIAL_ITEM = 12;

	// static SInitialItem initialItems[JOB_MAX_NUM][MAX_INITIAL_ITEM] = 
	// {
		// {
			// // WARRIOR
			// {EQUIPMENT,	0,	1,	11209, 0, 0, 0, 0, 0, 0}, // armor
			// {EQUIPMENT,	1,	1,	12209, 0, 0, 0, 0, 0, 0}, // head
			// {EQUIPMENT,	2,	1,	15009, 0, 0, 0, 0, 0, 0}, // shoes
			// {EQUIPMENT,	3,	1,	14009, 0, 0, 0, 0, 0, 0}, // bracelet
			// {EQUIPMENT,	4,	1,	19	 , 0, 0, 0, 0, 0, 0}, // weapon
			// {EQUIPMENT,	5,	1,	16009, 0, 0, 0, 0, 0, 0}, // necklace
			// {EQUIPMENT,	6,	1,	17009, 0, 0, 0, 0, 0, 0}, // earring
			// {EQUIPMENT,	10,	1,	13009, 0, 0, 0, 0, 0, 0}, // shield
			
			// {INVENTORY,	0,	1,	50052, /* sockets => */ 0, 0, 0, 0, 0, 0}, // At Kitab˝
			// {INVENTORY,	1,	1,	72701, /* sockets => */ 0, 0, 1800, 0, 0, 0}, // R¸zgar˝n Ayakkab˝lar˝
			// {INVENTORY,	2,	1,	39037, /* sockets => */ 0, 0, 1000000, 0, 0, 0}, // G¸ne˛ ÷z¸t¸ (K)
			// {INVENTORY,	3,	1,	39040, /* sockets => */ 0, 0, 1000000, 0, 0, 0}, // Ay ÷z¸t¸ (K)

		// },
		// {
			// // ASSASSIN
			// {EQUIPMENT,	0,	1,	11409, 0, 0, 0, 0, 0, 0}, // armor
			// {EQUIPMENT,	1,	1,	12349, 0, 0, 0, 0, 0, 0}, // head
			// {EQUIPMENT,	2,	1,	15009, 0, 0, 0, 0, 0, 0}, // shoes
			// {EQUIPMENT,	3,	1,	14009, 0, 0, 0, 0, 0, 0}, // bracelet
			// {EQUIPMENT,	4,	1,	1009 , 0, 0, 0, 0, 0, 0}, // weapon
			// {EQUIPMENT,	5,	1,	16009, 0, 0, 0, 0, 0, 0}, // necklace
			// {EQUIPMENT,	6,	1,	17009, 0, 0, 0, 0, 0, 0}, // earring
			// {EQUIPMENT,	10,	1,	13009, 0, 0, 0, 0, 0, 0}, // shield
		...
		// {
			// // SURA
			// {EQUIPMENT,	0,	1,	11609, 0, 0, 0, 0, 0, 0}, // armor
			// {EQUIPMENT,	1,	1,	12489, 0, 0, 0, 0, 0, 0}, // head
			// {EQUIPMENT,	2,	1,	15009, 0, 0, 0, 0, 0, 0}, // shoes
			// {EQUIPMENT,	3,	1,	14009, 0, 0, 0, 0, 0, 0}, // bracelet
			// {EQUIPMENT,	4,	1,	19	 , 0, 0, 0, 0, 0, 0}, // weapon
			// {EQUIPMENT,	5,	1,	16009, 0, 0, 0, 0, 0, 0}, // necklace
		...
		// {
			// // SHAMAN
			// {EQUIPMENT,	0,	1,	11809, 0, 0, 0, 0, 0, 0}, // armor
			// {EQUIPMENT,	1,	1,	12629, 0, 0, 0, 0, 0, 0}, // head
			// {EQUIPMENT,	2,	1,	15009, 0, 0, 0, 0, 0, 0}, // shoes
			// {EQUIPMENT,	3,	1,	14009, 0, 0, 0, 0, 0, 0}, // bracelet
			// {EQUIPMENT,	4,	1,	7009 , 0, 0, 0, 0, 0, 0}, // weapon
			// {EQUIPMENT,	5,	1,	16009, 0, 0, 0, 0, 0, 0}, // necklace
			// {EQUIPMENT,	6,	1,	17009, 0, 0, 0, 0, 0, 0}, // earring
			// {EQUIPMENT,	10,	1,	13009, 0, 0, 0, 0, 0, 0}, // shield
...
void CInputDB::PlayerLoad(LPDESC d, const char * data)
{
	...
	if (!SECTREE_MANAGER::instance().GetValidLocation(pTab->lMapIndex, pTab->x, pTab->y, lMapIndex, pos, d->GetEmpire()))
	{
		...
		std::auto_ptr<SQLMsg> pmsg2(DBManager::instance().DirectQuery(szQuery));
...
void CInputDB::SafeboxLoad(LPDESC d, const char * c_pData)
{
	...
	{
		d->GetCharacter()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¥Ÿ∏•∞≈∑°√¢¿Ã ø≠∏∞ªÛ≈¬ø°º≠¥¬ √¢∞Ì∏¶ ø≠ºˆ∞° æ¯Ω¿¥œ¥Ÿ." ) );
...
void CInputDB::SafeboxChangePasswordAnswer(LPDESC d, const char* c_pData)
{
	...
	if (p->flag)
	{
		d->GetCharacter()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<√¢∞Ì> √¢∞Ì ∫Òπ–π¯»£∞° ∫Ø∞Êµ«æ˙Ω¿¥œ¥Ÿ."));
	}
	else
	{
		d->GetCharacter()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<√¢∞Ì> ±‚¡∏ ∫Òπ–π¯»£∞° ∆≤∑»Ω¿¥œ¥Ÿ."));
	}
}
...
void CInputDB::AddMonarchMoney(LPDESC d, const char * data )
{
	...
	{
		if (number(1, 100) > 95) 
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("«ˆ¿Á %s ±π∞Ìø°¥¬ %u ¿« µ∑¿Ã ¿÷Ω¿¥œ¥Ÿ"), EMPIRE_NAME(Empire), CMonarch::instance().GetMoney(Empire));
	}
}
...
void CInputDB::DecMonarchMoney(LPDESC d, const char * data)
{
	...
	if (ch)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("«ˆ¿Á %s ±π∞Ìø°¥¬ %d ¿« µ∑¿Ã ¿÷Ω¿¥œ¥Ÿ"), EMPIRE_NAME(Empire), CMonarch::instance().GetMoney(Empire));
	}
}
...
void CInputDB::TakeMonarchMoney(LPDESC d, const char * data)
{
	...
	if (!CMonarch::instance().DecMoney(Money, Empire))
	{
		...
		LPCHARACTER ch = d->GetCharacter();
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("±π∞Ìø° µ∑¿Ã ∫Œ¡∑«œ∞≈≥™ µ∑¿ª ∞°¡Æø√ºˆ æ¯¥¬ ªÛ»≤¿‘¥œ¥Ÿ"));
	}
}
...
{
	char notice[256];
	snprintf(notice, sizeof(notice), LC_TEXT("%s¿« ±∫¡÷∞° %s ¥‘¿∏∑Œ ±≥√ºµ«æ˙Ω¿¥œ¥Ÿ."), EMPIRE_NAME(info->bEmpire), info->szName);
	SendNotice(notice);
}
