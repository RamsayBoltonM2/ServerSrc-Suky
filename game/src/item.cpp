...
LPITEM CItem::RemoveFromCharacter()
{
	...
	else
	{
		if (GetWindow() != SAFEBOX && GetWindow() != MALL)
		{
			...
			if (false == cell.IsDefaultInventoryPosition()) // 아니면 소지품에?
...
{
	return (m_pProto->bType == ITEM_ARMOR && (m_pProto->bSubType == ARMOR_WRIST || m_pProto->bSubType == ARMOR_NECK || m_pProto->bSubType == ARMOR_EAR));					// 2013년 2월 새로 추가된 '벨트' 아이템의 경우 기획팀에서 악세서리 소켓 시스템을 그대로 이용하자고 함.
}
