...
teleport_area:
	...
	int pc_point_change(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
	...
	int pc_update_cache(lua_State* L)
	{
		DWORD pid = (int) lua_tonumber(L, 1);
		db_clientdesc->DBPacketHeader(HEADER_GD_FLUSH_CACHE, 0, sizeof(DWORD));
		db_clientdesc->Packet(&pid, sizeof(DWORD));
		return 1;
	}
	...
	int pc_make_duel( lua_State* L )
	{
		sys_log(0, "QUEST make_duel %s to %s", lua_tostring(L, 1), lua_tostring(L, 2));

		LPCHARACTER pChar = CHARACTER_MANAGER::instance().FindPC(lua_tostring(L, 1));
		LPCHARACTER pVictimChar = CHARACTER_MANAGER::instance().FindPC(lua_tostring(L, 2));

		if ( pChar != NULL && pVictimChar != NULL )
		{
			///
			pChar->SetPKMode(PK_MODE_REVENGE);
			pVictimChar->SetPKMode(PK_MODE_REVENGE);

			///
			CPVPManager::instance().Insert(pChar, pVictimChar);
			CPVPManager::instance().Insert(pVictimChar, pChar);
	...
	int pc_send_damage_packet(lua_State* L)
	{
		CHARACTER * pc = quest::CQuestManager::instance().GetCurrentCharacterPtr();
		
		if (!lua_isnumber(L, 1) || !lua_isnumber(L, 2) || !pc) {
			return 0;
		}
		...
		{
			case 0:
				sendFlag = DAMAGE_NORMAL;
				break;
			case 1:
				sendFlag = DAMAGE_POISON;
				break;
			case 2:
				sendFlag = DAMAGE_DODGE;
				break;
			case 3:
				sendFlag = DAMAGE_BLOCK;
				break;
			case 4:
				sendFlag = DAMAGE_PENETRATE;
				break;
			case 5:
				sendFlag = DAMAGE_CRITICAL;
				break;
			default:
				sendFlag = DAMAGE_NORMAL;
		}
		
		pc->SendDamagePacket(pc, lua_tonumber(L, 1), sendFlag);
		
		return 1;
	}
	
	int pc_damage(lua_State* L)
	{
		CHARACTER* pc = quest::CQuestManager::instance().GetCurrentCharacterPtr();
		...
		DWORD vid = (DWORD) lua_tonumber(L, 1);
		int dam = lua_tonumber(L, 2);
		
		CHARACTER* pkVictim = CHARACTER_MANAGER::instance().Find(vid);
		
		if (pkVictim)
		{
			pkVictim->Damage(pc, dam, DAMAGE_TYPE_NORMAL);
		}
		
		return 0;
	}
	
	int pc_find_item_select(lua_State* L)
	{
		CHARACTER * pc = quest::CQuestManager::instance().GetCurrentCharacterPtr();
		
		if (!pc || !lua_isnumber(L, 1)) {
			return 0;
		}
		
		CItem * item = pc->FindSpecifyItem((DWORD) lua_tonumber(L, 1));
	...
	struct EjderKisi
	{
		...
		void operator()(LPENTITY ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (ch->IsPC())
				{
					if (!ch->IsPC() || ch->IsGM() || ch->IsObserverMode())
						return;
	...
	void RegisterPCFunctionTable()
	{
		luaL_reg pc_functions[] = 
		{
			...
			{ "update_cache",		pc_update_cache				},
			{ "open_shop",			pc_open_shop				},
			{ "set_max_health",		pc_set_max_health			},
			{ "make_duel",			pc_make_duel				},
			{ "send_damage_packet",	pc_send_damage_packet },
			{ "damage", pc_damage },
