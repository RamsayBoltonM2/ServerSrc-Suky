...
bool 		g_bAutoBackup = false;
bool 		g_bAutoClear = false;
	...
	*log_db = '\0';
	...
	while (fgets(buf, 256, fp))
	{
		...
		TOKEN("map_allow")
		{
			...
			fprintf(stderr, "Map allow size: %d/%d\n", max_allw_count, MAP_ALLOW_LIMIT);
			if(max_allw_count > MAP_ALLOW_LIMIT)
				fprintf(stderr, "Warning! More than %d MAP_ALLOW entries detected! Instability may occur!\n", MAP_ALLOW_LIMIT);
