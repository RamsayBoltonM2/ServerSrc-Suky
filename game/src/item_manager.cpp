...
bool ITEM_MANAGER::GetDropPct(LPCHARACTER pkChr, LPCHARACTER pkKiller, OUT int& iDeltaPercent, OUT int& iRandRange)
{
	...
	// ADD_PREMIUM
	if (pkKiller->IsEquipUniqueGroup(UNIQUE_GROUP_DOUBLE_ITEM)) //any kind of equipped gloves
		iDeltaPercent += iDeltaPercent;
	// END_OF_ADD_PREMIUM

	int bonus = 0;
	if (pkKiller->IsEquipUniqueItem(UNIQUE_ITEM_DOUBLE_ITEM)) {
		//irremovable gloves + mall
		bonus = 100;
	}
	else if (pkKiller->IsEquipUniqueItem(UNIQUE_ITEM_DOUBLE_ITEM) || pkKiller->IsEquipUniqueGroup(UNIQUE_GROUP_DOUBLE_ITEM)) { 
...
void ITEM_MANAGER::CreateQuestDropItem(LPCHARACTER pkChr, LPCHARACTER pkKiller, std::vector<LPITEM> & vec_item, int iDeltaPercent, int iRandRange)
{
	...
	//육각보합
	if (GetDropPerKillPct(100, g_iUseLocale ? 1000 : 800, iDeltaPercent, "2006_drop") >= number(1, iRandRange))
	{
	...
	if (GetDropPerKillPct(1, g_iUseLocale ? 100 : 1000, iDeltaPercent, "valentine_drop") >= number(1, iRandRange))
	{
	...
	if (quest::CQuestManager::instance().GetEventFlag("new_valentine_drop"))
	{
		const static DWORD valentine_items[2] = { 71146, 71147 };
		DWORD dwVnum = valentine_items[number(0, 1)];
		
		if (pkChr->GetRaceNum() >= 8001 && pkChr->GetRaceNum() <= 8027)
		{
			int iPercent = number(1,50);
	...
	if ( GetDropPerKillPct(100, g_iUseLocale ? 1000 : 800, iDeltaPercent, "halloween_drop") >= number(1, iRandRange) )
	{
	...
	if ( GetDropPerKillPct(100, g_iUseLocale ? 1000 : 800, iDeltaPercent, "ramadan_drop") >= number(1, iRandRange) )
	{
	...
	if ( GetDropPerKillPct(100, g_iUseLocale ? 1000 : 800, iDeltaPercent, "easter_drop") >= number(1, iRandRange) )
	{
	...
	// 월드컵 이벤트
	if ( GetDropPerKillPct(100, g_iUseLocale ? 1000 : 800, iDeltaPercent, "football_drop") >= number(1, iRandRange) )
	{
	...
	// 화이트 데이 이벤트
	if (GetDropPerKillPct(100, g_iUseLocale ? 1000 : 800, iDeltaPercent, "whiteday_drop") >= number(1, iRandRange))
	{
